  ...
  public:
    ...
    void for_component(const vector<FunctionPtr> &component, const vector<FunctionPtr> &edges) override {
      ...
      for (FunctionPtr f : component) {
        empty &= modified_global_vars[f].empty();
      }
      for (FunctionPtr f : edges) {
        empty &= f->bad_vars == nullptr;
      }
      if (empty) {
        return;
      }
      ...
      if (largest_e) {
        bool all_in_largest_e = true;
        for (FunctionPtr f : component) {
          for (VarPtr v_in_self : modified_global_vars[f]) {
            if (largest_e->find(v_in_self) == largest_e->end()) {
              all_in_largest_e = false;
              break;
            }
          }
        }
        for (FunctionPtr e : edges) {
          if (e->bad_vars != largest_e && e->bad_vars != nullptr && all_in_largest_e) {
            for (VarPtr v_in_e : *e->bad_vars) {
              if (largest_e->find(v_in_e) == largest_e->end()) {
                all_in_largest_e = false;
                break;
              }
            }
